...
Write-Host '[+] Install root ca certificates'
Import-Certificate `
    -FilePath "c:\OEM\certs\${DOMAIN_NAME_1}.ca.der" `
    -CertStoreLocation Cert:\LocalMachine\Root | Out-Null
Import-Certificate `
    -FilePath "c:\OEM\certs\${DOMAIN_NAME_2}.ca.der" `
    -CertStoreLocation Cert:\LocalMachine\Root | Out-Null
Import-Certificate `
    -FilePath "c:\OEM\certs\${DOMAIN_NAME_3}.ca.der" `
    -CertStoreLocation Cert:\LocalMachine\Root | Out-Null

switch (${env:COMPUTERNAME}) {
    'dc1' {$DOMAIN_NAME = $DOMAIN_NAME_1}
    'dc2' {$DOMAIN_NAME = $DOMAIN_NAME_2}
    'dc3' {$DOMAIN_NAME = $DOMAIN_NAME_3}
...
if (('web' -eq $env:COMPUTERNAME) -or ($env:COMPUTERNAME -like 'dc*')) {
