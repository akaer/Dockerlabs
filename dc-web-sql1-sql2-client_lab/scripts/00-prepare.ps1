...
$networkConfigs = @(
    ...
    @{
        Name         = 'Sql1'
        MAC          = $SQL1_NETWORK_MAC
        ComputerName = $SQL1_COMPUTERNAME
    },
    @{
        Name         = 'Sql2'
        MAC          = $SQL2_NETWORK_MAC
        ComputerName = $SQL2_COMPUTERNAME
    },
    @{
        Name         = 'Client'
...
$configuredMACs = foreach ($config in $networkConfigs) {
    ...
    if ($adapter) {
        ...
        Rename-Computer -NewName $config.ComputerName -Force | Out-Null
...
'Please reboot container' | Out-File "\\host.lan\Data\state\${ComputerName}_reboot.txt"
